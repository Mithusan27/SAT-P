...
class PetriNet:
    ...
    def smtlib_set_initial_marking(self, k: Optional[int] = None) -> str:
        ...
        for pl in self.places:
            smt_input += "(= {}@{} {})".format(pl,k,self.initial_marking[pl])
        smt_input +="))"
        return smt_input
    ######################
    ...
    def smtlib_transition_relation(self, k: int, k_prime: int) -> str:
        ...
        -------
        str
            SMT-LIB format encoding the transition relation.
        """
        def smtlib_transition_enable(k: int, transition: str) -> str:
            conditions = [
            f"(>= {place}@{k} {self.pre[transition].get(place, 0)})"
            for place in self.places
           ]
            return f"(and {' '.join(conditions)})"
        
        def smtlib_transition_delta(k: int, k_prime: int, transition: str) -> str:
            upd = [
            f"(= {place}@{k_prime} (+ (- {place}@{k} {self.pre[transition].get(place, 0)}) {self.post[transition].get(place, 0)}))"
            for place in self.places
            ]
            return f"(and {' '.join(upd)})"

        transition_cond = [
        f"(and {smtlib_transition_enable(k, t)} {smtlib_transition_delta(k, k_prime, t)})"
        for t in self.transitions
        ]
        smt_input = f"(assert (or {' '.join(transition_cond)}))"
        return smt_input
