...
class KInduction(AbstractChecker):
    ...
    induction_queue : Queue of int, optional
        Queue for the exchange with KINDUCTION.
    solver : Z3
        SMT solver (Z3).
    """
    ...
    def prove_helper(self) -> int:
        ...
        """
        info("[KINDUCTION] > Initialization")

        info("[KINDUCTION] > Declaration of the places from the Petri net (iteration: 0)")
        self.solver.write(self.ptnet.smtlib_declare_places(0))

        info("[KINDUCTION] > Push")
        self.solver.push()

        info("[KINDUCTION] > Formula to check the satisfiability (iteration: 0)")
        ...
        while not self.solver.check_sat():
            ...
            self.solver.pop()           
            k += 1
            info("[KINDUCTION] > k = {}".format(k))

            info("[KINDUCTION] > Declaration of the places from the Petri net (iteration: {})".format(k))
            self.solver.write(self.ptnet.smtlib_declare_places(k))

            info("[KINDUCTION] > Transition relation: {} -> {}".format(k - 1, k))
            self.solver.write(self.ptnet.smtlib_transition_relation(k - 1, k))
            info("[KINDUCTION] > Push")
            self.solver.push()
        
            info("[KINDUCTION] > Formula to check the satisfiability (iteration: {})".format(k))
